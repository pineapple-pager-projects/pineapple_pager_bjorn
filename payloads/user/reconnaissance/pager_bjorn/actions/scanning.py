...
class NetworkScanner:
    ...
    class ScanPorts:
        ...
        def start(self):
            ...
            sd.bjornstatustext2 = "Discovering hosts..."
            ...
            sd.bjornstatustext2 = f"Found {total_ips} hosts"
            self.logger.info(f"Found {total_ips} hosts")
            self._write_netkb_incremental()

            # Phase 2: Port scanning
            sd.bjornstatustext2 = f"Scanning ports on {total_ips} hosts..."
            ...
            sd.bjornstatustext2 = f"{len(self.all_ports)} open ports found"
            self._write_netkb_incremental()
        ...
        def _run_version_detection(self):
            ...
            for i, (ip, ports) in enumerate(hosts_with_ports):
                ...
                sd.bjornstatustext2 = f"Identifying services {i+1}/{total}"
                try:
            ...
            sd.bjornstatustext2 = f"Services identified on {total} hosts"
        ...
        def _run_os_detection(self):
            ...
            for i, (ip, ports) in enumerate(hosts_to_scan):
                port_str = ','.join(str(p) for p in sorted(ports))
                sd.bjornstatustext2 = f"OS detection {i+1}/{total}"
                try:
            ...
            sd.bjornstatustext2 = f"OS detected on {total} hosts"
