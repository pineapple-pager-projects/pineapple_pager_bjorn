...
function adjustNetworkFontSize(change) {
    ...
    // Also set on the table element inside
    const table = document.querySelector('#network-table table');
    if (table) {
        table.style.fontSize = fontSize + 'px';
    }
    // And on all cells to ensure it takes effect
    document.querySelectorAll('#network-table th, #network-table td').forEach(cell => {
