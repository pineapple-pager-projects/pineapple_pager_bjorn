...
Pager payloads can seamlessly hand off to each other through a shared protocol. When a compatible payload is detected, an "Exit to [App]" option appears in the pause menu. This system is used by PagerBjorn, PagerGotchi, and any custom payload that follows this convention.
...
To let **other apps** launch yours, create a `launch_<yourapp>.sh` file and place it in **their** payload directory. For example, PagerBjorn includes `launch_pagergotchi.sh`, and PagerGotchi includes `launch_bjorn.sh`.
...
- Each app skips its own self-launcher automatically (e.g., PagerBjorn skips `launch_bjorn.sh`)
...
```
/root/payloads/user/reconnaissance/pager_bjorn/launch_myapp.sh    # So Bjorn can launch your app
/root/payloads/user/reconnaissance/pagergotchi/launch_myapp.sh    # So Pagergotchi can launch your app
/root/payloads/user/reconnaissance/my_app/launch_bjorn.sh         # So your app can launch Bjorn
...
- **PagerBjorn** — `display.py:discover_launchers()`, `payload.sh` handoff loop
